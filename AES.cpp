// ADAPTED FROM : http://www.efgh.com/software/rijndael.txt
...
// Sbox but every 8 bits element is repeated 4 times to make a 32 bits element
...
// Inverse Sbox but every 8 bits element is repeated 4 times to make a 32 bits element

static const uint32_t ISbox[256] =
{
...
// Te : Table precalculating MixColumn with SubBytes for encryption
...
// Td : Table precalculating MixColumn with SubBytes for decryption
...
int expandEncryptKey(uint32_t *rk, const uint64_t *key, int keybits)
{
  ...
  if (keybits == 128)
  {
    for (;;)
    {
      temp  = rk[3];
      rk[4] = rk[0] ^
        (Sbox[(temp >> 16) & 0xff] & 0xff000000) ^
        (Sbox[(temp >>  8) & 0xff] & 0x00ff0000) ^
        (Sbox[(temp      ) & 0xff] & 0x0000ff00) ^
        (Sbox[(temp >> 24)       ] & 0x000000ff) ^
        rcon[i];
      rk[5] = rk[1] ^ rk[4];
      rk[6] = rk[2] ^ rk[5];
      rk[7] = rk[3] ^ rk[6];
...
int expandDecryptKey(uint32_t *rk, const uint64_t *key, int keybits)
{
  ...
  nrounds = expandEncryptKey(rk, key, keybits);
  ...
  for (i = 1; i < nrounds; i++)
  {
    rk += 4;
    rk[0] =
      Td0[Sbox[(rk[0] >> 24)       ] & 0xff] ^
      Td1[Sbox[(rk[0] >> 16) & 0xff] & 0xff] ^
      Td2[Sbox[(rk[0] >>  8) & 0xff] & 0xff] ^
      Td3[Sbox[(rk[0]      ) & 0xff] & 0xff];
    rk[1] =
      Td0[Sbox[(rk[1] >> 24)       ] & 0xff] ^
      Td1[Sbox[(rk[1] >> 16) & 0xff] & 0xff] ^
      Td2[Sbox[(rk[1] >>  8) & 0xff] & 0xff] ^
      Td3[Sbox[(rk[1]      ) & 0xff] & 0xff];
    rk[2] =
      Td0[Sbox[(rk[2] >> 24)       ] & 0xff] ^
      Td1[Sbox[(rk[2] >> 16) & 0xff] & 0xff] ^
      Td2[Sbox[(rk[2] >>  8) & 0xff] & 0xff] ^
      Td3[Sbox[(rk[2]      ) & 0xff] & 0xff];
    rk[3] =
      Td0[Sbox[(rk[3] >> 24)       ] & 0xff] ^
      Td1[Sbox[(rk[3] >> 16) & 0xff] & 0xff] ^
      Td2[Sbox[(rk[3] >>  8) & 0xff] & 0xff] ^
      Td3[Sbox[(rk[3]      ) & 0xff] & 0xff];
  }
  return nrounds;
}
...
void decryptAES(const uint32_t *rk, int nrounds, const uint64_t cipher[16], uint64_t plain[16])
{
  ...
  */
  s0 =
    (ISbox[(t0 >> 24)       ] & 0xff000000) ^
    (ISbox[(t3 >> 16) & 0xff] & 0x00ff0000) ^
